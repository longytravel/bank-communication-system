...
def render_generate_plans_tab():
 ...
 st.metric("[VIDEO] Video Eligible", video_eligible_count)
 else:
 ...
 st.warning("WARNING This will make real API calls and may take 2-3 minutes for 20 customers")
 ...
 with col2:
 if st.button(
 f"[TARGET] Generate Plans for {len(filtered_customers)} Customers",
 type="primary",
...
def generate_real_communication_plans(customers, letter, options, batch_size):
 ...
 <h3 style="margin-top: 0; color: white;">[AI] AI Generation in Progress (Real API Calls)</h3>
 ...
 status.text("OK All AI content generated successfully!")
...
def generate_demo_communication_plans(customers, letter, options):
 ...
 st.markdown("""
 <div style="background: #3B82F6; color: white; border-radius: 8px; padding: 1.5rem; margin: 1rem 0;">
 <h3 style="margin-top: 0; color: white;">[AI] Generating Demo Plans</h3>
 <p style="color: rgba(255,255,255,0.9); margin-bottom: 0;">
 ...
 status.text("OK All plans generated successfully!")
 progress.progress(1.0)
 time.sleep(1)
...
def show_generation_success():
 ...
 <h3 style="margin-top: 0; color: white;">OK Generation Complete!</h3>
 <p style="color: rgba(255,255,255,0.9); margin-bottom: 0;">
 ...
 with col1:
 if st.button("[DATA] View Results", use_container_width=True):
...
def generate_template_content(name: str, category: str, classification_type: str, upsell_eligible: bool, 
 ...
 if include_video:
 notes.append(f"[VIDEO] Premium video message included ({video_tier} tier)")
 
 if classification_type == "REGULATORY":
 if category in ["Digital-first self-serve", "Assisted-digital"]:
 notes.append("OK Using EMAIL as durable medium (saves 1.46 vs letter)")
 else:
 notes.append("[LETTER] Using LETTER as durable medium for traditional/vulnerable customer")
...
def create_real_ai_content_for_customer(customer: Dict, classification_type: str, letter_content: str, cost_manager, api_manager, options: Dict) -> Dict:
 ...
 YES Balance: You MUST mention " {customer.get('account_balance', 0):,}" explicitly
 YES Age: You MUST reference "{customer.get('age', 'Unknown')} years" when relevant
 YES Usage: You MUST mention "{customer.get('digital_logins_per_month', 0)} logins per month"
 YES Products: You MUST list these specific products: {', '.join(upsell_products) if upsell_products else 'standard services'}
 
 EXAMPLE OF WHAT YOU MUST DO:
 NO WRONG: "As a valued customer..." 
 OK RIGHT: "With your {customer.get('account_balance', 0):,} balance..."
 
 NO WRONG: "You're eligible for premium services"
 OK RIGHT: "You qualify for: {', '.join(upsell_products)}"
 
 NO WRONG: "As an active user..."
 OK RIGHT: "Your {customer.get('digital_logins_per_month', 0)} monthly logins show..."
 """
