...
# ADD THIS CODE TO customer_plans_ui.py
# 
# INSTRUCTIONS:
# 1. Find the function called "render_individual_customer_details" (around line 1400)
# 2. Look for where the Video Message section ends (around line 1500)
# 3. Add this code RIGHT AFTER the video section
# 4. Make sure the indentation matches (it should be at the same level as the video if statement)
    ...
    if 'in_app' in selected_plan['channels'] and 'in_app' in content:
        with st.expander("📱 In-App Notification", expanded=False):
            ...
            # Check if in_app_content is a dict or string
    ...
    if 'email' in selected_plan['channels'] and 'email' in content:
        with st.expander("📧 Email Communication", expanded=False):
            ...
            if isinstance(email_content, dict):
                ...
                # If there's HTML content, show preview option
    ...
    if 'sms' in selected_plan['channels'] and 'sms' in content:
        with st.expander("💬 SMS Message", expanded=False):
            ...
            # Display SMS with character count
